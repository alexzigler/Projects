...
xdot=@(x,u)[x(3);...
      x(4);...
      -delta1/m1*x(3)-k1/m1*x(1)+miu1/m1 * x(1)^3 - k2/m1 * (x(1)-x(2))+miu2/m1*(x(1)-x(2))^3+u(1)/m1;...
      -delta2/m2*x(4)-k2/m2*(x(2)-x(1))+miu2/m2*(x(2)-x(1))^3+u(2)/m2];
...
for k=1:endsim
    ...
    % control action amplitude constraints
    
    E=repmat(eye(m),[Np,1]);
    H=kron(tril(ones(Np)),eye(m)); % creates a diagonal matrix with identity block matrices
    a_ineq=[-H;H];
    umin=-4;
    umax=4;
    if k==1
        b_ineq=[-umin+E*[0;0];umax-E*[0;0]];
    else
        b_ineq=[-umin+E*uv(:,k-1);umax-E*uv(:,k-1)];
    end
    
    Hqp=Z'*Q*Z+R;
    fqp=(-(r-W*xa_k)'*Q*Z)';
    deltaUstar=quadprog(Hqp,fqp,a_ineq,b_ineq);
